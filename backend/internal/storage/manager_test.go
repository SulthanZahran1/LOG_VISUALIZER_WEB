...
func TestLocalStore_PersistsMetadataAcrossRestart(t *testing.T) {
	tempDir := t.TempDir()
	...
	info, err := store.Save("persisted.log", strings.NewReader("persist me"))
	if err != nil {
		t.Fatalf("Failed to save file: %v", err)
	}

	reloaded, err := NewLocalStore(tempDir)
	if err != nil {
		t.Fatalf("Failed to recreate store: %v", err)
	}

	got, err := reloaded.Get(info.ID)
	if err != nil {
		t.Fatalf("Failed to load persisted metadata: %v", err)
	}
	if got.Name != "persisted.log" {
		t.Errorf("Expected persisted name, got %q", got.Name)
	}

	files, err := reloaded.List(10)
	if err != nil {
...
func TestLocalStore_Delete(t *testing.T) {
	...
	t.Run("deletes persisted metadata", func(t *testing.T) {
		store, cleanup := createTestStore(t)
		defer cleanup()
		...
		metaPath := filepath.Join(store.metaDir, info.ID+".json")
		if _, err := os.Stat(metaPath); os.IsNotExist(err) {
			t.Fatal("Metadata file should exist before deletion")
		}
