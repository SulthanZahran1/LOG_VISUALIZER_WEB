...
func TestDuckStore_FlushAndQuery(t *testing.T) {
	...
	t.Run("queries with signal-name and device-id filters", func(t *testing.T) {
		store, cleanup := createTestStore(t)
		defer cleanup()
		...
		store.AddEntry(createTestEntry("PLC-01", "Signal2", baseTime.Add(time.Second), false, ""))
		store.AddEntry(createTestEntry("PLC-02", "Signal1", baseTime.Add(2*time.Second), true, ""))
		store.AddEntry(createTestEntry("PLC-03", "Signal3", baseTime.Add(3*time.Second), false, ""))
		...
		ctx := context.Background()
		params := QueryParams{
			SignalNames: []string{"Signal1"},
			DeviceIDs:   []string{"PLC-02"},
		}
		...
		if entries[0].DeviceID != "PLC-02" || entries[0].SignalName != "Signal1" {
			t.Fatalf("Expected PLC-02 Signal1, got %s %s", entries[0].DeviceID, entries[0].SignalName)
		}
	})
