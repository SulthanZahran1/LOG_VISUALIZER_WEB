# CIM Visualizer Technical Documentation

Last updated: 2026-02-27

## 1. Project Overview and Purpose

CIM Visualizer is a Chrome-targeted web application for PLC/AMHS log analysis. It provides a unified workflow for ingesting industrial logs, parsing them into normalized signal timelines, and visualizing the result in multiple synchronized views.

Primary goals:
- Parse multiple log formats used in PLC/AMHS environments.
- Support very large files with memory-efficient server-side processing.
- Provide synchronized analysis surfaces:
  - Log table (search/filter/sort/selection).
  - Waveform/timing diagram.
  - Map-based state visualization.
  - Transition and transfer-heatmap views.
- Support both single-file parsing and merged multi-file sessions.

Core runtime model:
- Upload file(s) -> create parse session -> stream/query parsed entries -> render across views.

---

## 2. Full Stack Architecture (Go Backend + Preact Frontend)

### 2.1 Technology Stack

| Layer | Technology |
|---|---|
| Backend language/runtime | Go 1.24 |
| Backend web framework | Echo v4 |
| Backend analytics store | DuckDB (`github.com/marcboeker/go-duckdb`) |
| Backend serialization | JSON + SSE + WebSocket + Msgpack endpoint |
| Frontend framework | Preact 10 + TypeScript |
...
- Expose SSE streams for parse progress and entry streaming.
- Expose WebSocket upload protocol at `/api/ws/uploads`.
- Persist uploads/chunks locally.
- Parse logs into session-scoped data stores.
- Query parsed data efficiently using DuckDB for large workloads.
- Serve embedded frontend assets when built in embedded mode.

Key internal packages:
- `backend/internal/api`: modular handlers (`health`, `upload`, `parse`, `map`, `carrier`) and WebSocket protocol handler.
- `backend/internal/parser`: parser registry, format parsers, DuckDB-backed store, map/rules parsers, merge logic.
- `backend/internal/session`: parse-session lifecycle, lookup/query APIs, cleanup, persistent parsed DB reuse.
- `backend/internal/storage`: local file and chunk store.
- `backend/internal/upload`: async chunk-assembly/decompression job manager.
- `backend/internal/config`: XML config loader (`PLCLogVisualizer.exe.config`).

Server startup flow:
1. Resolve executable directory and load XML config.
2. Ensure data/upload/temp/parsed directories exist.
3. Initialize local file store.
4. Initialize session manager and background cleanup ticker.
5. Initialize upload manager.
6. Initialize handlers and WebSocket handler.
7. Register routes and middleware.
8. Optionally register embedded static frontend routes.
9. Start HTTP server with configured timeouts.

Middleware behavior highlights:
- Structured logging with optional suppression for high-frequency status/progress routes.
- Panic recovery middleware.
- Timeout middleware (skips streaming/upload/entries/SSE routes).
- Optional gzip compression (skips SSE).
- Body size limit from XML config.
- CORS differs between embedded mode (config-driven) and dev mode (localhost allowlist).
...
- Upload and parse-session orchestration.
- Query and render parsed data in multiple views.
- Maintain synchronized playback/time navigation between waveform and map.
- Manage large-data server-side query mode vs small-data client-side mode.

Top-level frontend structure:
- `frontend/src/api`: typed API wrappers + upload/WebSocket helpers.
- `frontend/src/components`: UI components by domain (`file`, `log`, `waveform`, `map`, `settings`, `transition`).
- `frontend/src/stores`: modular signal stores (`log`, `waveform`, `map`) plus legacy re-export entrypoints.
- `frontend/src/views`: `HomeView`, `MapViewer` and app-level view composition.

Store architecture pattern (applies to `log`, `waveform`, `map`):
- `state.ts`: signals + computed values.
- `actions.ts`: mutations + async operations.
- `effects.ts`: reactive side effects.
- `types.ts`: module-local contracts.
- `index.ts`: export surface.

Legacy compatibility entrypoints:
- `frontend/src/stores/logStore.ts`
- `frontend/src/stores/waveformStore.ts`
- `frontend/src/stores/mapStore.ts`

These re-export modular stores and preserve older import paths.

### 2.4 High-Level Data Flow

1. User uploads file(s) (HTTP or WebSocket).
2. Backend stores file and starts parse session.
3. Session manager detects parser and parses to:
   - DuckDB-backed store for PLC debug and merged sessions.
   - In-memory parsed log for smaller/legacy paths.
4. Frontend polls parse status/SSE until complete.
5. Frontend fetches entries/signals/categories and hydrates stores.
6. Views query slices (pagination, chunk windows, values-at-time, boundaries) as needed.
7. Map and waveform synchronize around shared time cursor when enabled.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/health` | Returns service health and backend version. |

### 3.2 WebSocket Upload Channel

| Method | Path | Purpose |
|---|---|---|
| GET | `/ws/uploads` | WebSocket endpoint for chunked uploads and file-type specific uploads. |
...
### 3.3 Files and Upload

| Method | Path | Purpose |
|---|---|---|
| POST | `/files/upload` | Base64 JSON file upload. |
| POST | `/files/upload/binary` | Multipart binary upload. |
| POST | `/files/upload/chunk` | Upload one chunk for chunked flow. |
| POST | `/files/upload/complete` | Finalize chunked upload and start async processing job. |
| GET | `/files/upload/:jobId/status` | Upload job status SSE stream (currently stubbed). |
| GET | `/files/recent` | List recent non-map/rules files. |
| GET | `/files/:id` | File metadata by id. |
| DELETE | `/files/:id` | Delete file (route only registered when `AllowFileDeletion=true`). |
...
| Method | Path | Purpose |
|---|---|---|
| POST | `/parse` | Start parse (`fileId` or merged `fileIds`). |
| GET | `/parse/:sessionId/status` | Session status/progress summary. |
| GET | `/parse/:sessionId/progress` | SSE parse progress stream. |
| GET | `/parse/:sessionId/entries` | Paginated entries query endpoint. |
| GET | `/parse/:sessionId/entries/msgpack` | Msgpack endpoint (currently delegates JSON handler). |
| GET | `/parse/:sessionId/stream` | SSE streamed entry batches. |
| POST | `/parse/:sessionId/chunk` | Time-window entry fetch, optional signals filter in body. |
| POST | `/parse/:sessionId/chunk-boundaries` | Boundary values before/after viewport for waveform continuity. |
| GET | `/parse/:sessionId/signals` | Unique signal keys (`deviceId::signalName`). |
| GET | `/parse/:sessionId/signal-types` | Signal type map by signal key. |
| GET | `/parse/:sessionId/categories` | Unique categories. |
| GET | `/parse/:sessionId/at-time` | Latest signal values at/before timestamp. |
| GET | `/parse/:sessionId/index-of-time` | Row index nearest timestamp with active filters/sort. |
| GET | `/parse/:sessionId/time-tree` | Date/hour/minute buckets for jump navigation. |
| POST | `/parse/:sessionId/keepalive` | Extend session lifetime for active viewing. |

Common parse-query filters supported by backend query builder:
- `search`
- `regex`
- `caseSensitive`
...
- `signals` or `signal` (format: `deviceId::signalName`)
- `signalType` or `type`
- `sortColumn` or `sort`
- `sortDirection` or `order`

### 3.5 Map, Rules, Carrier

| Method | Path | Purpose |
|---|---|---|
| GET | `/map/layout` | Return currently active map layout. |
| POST | `/map/upload` | Upload map XML and set active map. |
| POST | `/map/active` | Set active map by id (`mapId`), supports `default:*` ids. |
| GET | `/map/rules` | Return currently active rules payload. |
| POST | `/map/rules` | Upload rules YAML and set active rules. |
| GET | `/map/files/recent` | Return recent XML/YAML files. |
| GET | `/map/defaults` | List packaged default maps from `data/defaults/maps`. |
| POST | `/map/defaults/load` | Activate packaged default map by name. |
| POST | `/map/carrier-log` | Upload carrier log file. |
| GET | `/map/carrier-log` | Carrier log status/info. |
| GET | `/map/carrier-log/entries` | Carrier entries, optional time-range filtering. |

### 3.6 Validation Rules Config

| Method | Path | Purpose |
|---|---|---|
| GET | `/config/validation-rules` | Returns validation config payload (currently empty array). |
| PUT | `/config/validation-rules` | Update validation config (currently not implemented). |

---

## 4. Data Models and Storage (DuckDB, Parsers)

### 4.1 Core Backend Models
...
- `category?: string`
- `sourceId?: string` (for merged sessions)

`ParseSession`:
- Identifiers: `id`, `fileId`, optional `fileIds[]`.
- Status: `pending|parsing|complete|error`.
- Runtime metrics: `progress`, `entryCount`, `signalCount`, `processingTimeMs`.
- Time bounds: `startTime`, `endTime` (Unix ms).
- Parser metadata: `parserName`.
- Parse errors list.
...
Map/rules/carrier models:
- `MapLayout` with `objects: Record<string, MapObject>`.
